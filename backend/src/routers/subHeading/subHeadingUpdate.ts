      ...
      const updated = await subHeadingRepository.findByIdAndUpdate(new Types.ObjectId(input.id),
        {
          ...
          bookId: new Types.ObjectId(input.updates.bookId),
          chapterId: new Types.ObjectId(input.updates.chapterId),
          headingId: new Types.ObjectId(input.updates.headingId),
