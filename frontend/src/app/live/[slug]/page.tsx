...
const PAGE_TITLE = 'MCQ Detail | GrowLearnHub';

export default function LiveMcqDetailPage() {
    ...
    return (
      <UserLayout title={PAGE_TITLE}>
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          ...
          <p className="text-foreground/60 font-medium">Loading question...</p>
        </div>
      </UserLayout>
    );
  }
    ...
    return (
      <UserLayout title={PAGE_TITLE}>
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
          ...
          <p className="text-foreground/60 text-sm">{error?.message || 'The requested question does not exist.'}</p>
  ...
  return (
    <UserLayout title={mcq.question ?? PAGE_TITLE}>
      <article className="space-y-6 max-w-2xl">
        ...
        <div className="rounded-xl border border-border bg-background p-6 shadow-sm">
          ...
          <div className="flex items-start gap-3 mb-6">
            <span className="shrink-0 w-8 h-8 bg-primary/10 text-primary rounded-lg flex items-center justify-center font-semibold text-sm">
              Q
            </span>
            <h2 className="text-lg font-semibold text-foreground leading-relaxed">{mcq.question}</h2>
          </div>
          ...
          {mcq.options?.length > 0 ? (
            ...
            <p className="text-center py-8 text-foreground/40 text-sm">No options available</p>
          )}
        ...
        {!isAnswered && (
          <p className="text-xs text-foreground/40 text-center">Select an option to check your answer</p>
        )}
      </article>
    </UserLayout>
  );
