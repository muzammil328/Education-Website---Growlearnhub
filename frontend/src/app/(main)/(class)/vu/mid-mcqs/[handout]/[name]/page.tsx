...
export default function Page({ params }: { params: Promise<{ handout: string; name: string }> }) {
  ...
  React.useEffect(() => {
    params.then(p => { setHandoutSlug(p.handout); setNameSlug(p.name); });
  }, [params]);
  ...
  return (
    <UserLayout
      title={`${handoutName} – ${sourceName} | VU Mid Exam Handouts – GrowLearnHub`}
      canonical={`/vu/mid-mcqs/${handoutSlug}/${nameSlug}/`}
      url={`https://growlearnhub.com/vu/mid-mcqs/${handoutSlug}/${nameSlug}/`}
    >
      <article className="max-w-none">
        <header className="mb-4">
          <div className="flex items-center gap-2 flex-wrap">
            <Link href={`/vu/mid-mcqs/${handoutSlug}`} className="flex items-center gap-1 text-sm text-muted-foreground hover:text-primary transition-colors">
          ...
          {isLoading ? (
            ...
            <div className="rounded-lg border border-border overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3 bg-muted/50 border-b border-border">
                <span className="text-sm font-medium text-foreground">{sourceName}</span>
                <a href={currentLink.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 rounded-md bg-primary px-4 py-1.5 text-xs font-medium text-primary-foreground hover:bg-primary/90 transition-colors">
                  <ExternalLink className="h-3.5 w-3.5" />
                  Open in new tab
                </a>
              </div>
              <iframe src={currentLink.url} className="w-full" style={{ height: '85vh', minHeight: 500 }} title={`${handoutName} – ${sourceName}`} sandbox="allow-scripts allow-same-origin allow-forms allow-popups" />
            </div>
          )}
        </section>
      </article>
