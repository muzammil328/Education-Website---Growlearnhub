...
export function generateMetadata({ params }: { params: Params }): Metadata {
  ...
  if (!classItem || !board || !subject) {
    return {
      title: 'Class 9 Pairing Scheme Not Found | GrowLearnHub',
      description: 'Requested Class 9 pairing scheme page could not be found.',
    };
  }

  const canonical = `/class-9/pairing-scheme/${board.slug}/${subject.slug}/`;
      ...
      <article className="max-w-none">
        ...
        <section className="mt-8">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {previous && (
              <Link
                href={`/class-9/pairing-scheme/${board.slug}/${previous.slug}`}
            ...
            <Link
              href={`/class-9/pairing-scheme/${board.slug}`}
              className="rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted"
            >
              All Subjects
            </Link>
            {next && (
              <Link
                href={`/class-9/pairing-scheme/${board.slug}/${next.slug}`}
                className="rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted"
              >
                Next: {next.name}
              </Link>
