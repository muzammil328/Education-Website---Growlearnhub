...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  if (!book) {
    return {
      title: 'Class 9 Book Medium Not Found | GrowLearnHub',
      description: 'Requested Class 9 book medium could not be found.',
    };
  }
  ...
  const canonical = `/class-9/books/${book.slug}/${medium.slug}/`;
  const title = `${bookName} Class 9 Book PDF (${medium.name})`;
  const description = `Read ${bookName} Class 9 textbook in ${medium.name}. Use chapter-wise links or view the full PDF online.`;
  ...
  return generatePageMetadata({
    ...
    canonical,
    keywords: [
      `${bookName.toLowerCase()} class 9 ${medium.name.toLowerCase()} pdf`,
      `${bookName.toLowerCase()} class 9 book pdf`,
      'class 9 chapter wise book',
...
export default async function Page({ params }: { params: Params }) {
  ...
  return (
    <UserLayout
      title={`${bookName} Class 9 Book (${medium.name})`}
      image="/9th/class_9_book_growlearnhub.png"
      canonical={canonical}
      url={url}
    >
      <article className="max-w-none">
        <section className="mb-8">
          <p className="lead text-foreground/90">
            Access <strong>{bookName}</strong> Class 9 textbook in <strong>{medium.name}</strong>.
            Use chapter-wise links below for quick navigation, or read the full PDF in one place.
          </p>
          <p className="text-foreground/80 mt-4">
            Go back to{' '}
            <Link
              href={`/class-9/books/${book.slug}`}
            ...
            <div className="my-6 grid grid-cols-1 gap-4 md:grid-cols-2">
              {chaptersForMedium.map(chapter => (
                <CardSmall
                  ...
                  title={chapter.name}
                  link={`class-9/books/${book.slug}/${medium.slug}/${chapter.slug}`}
                />
              ))}
            </div>
          </section>
