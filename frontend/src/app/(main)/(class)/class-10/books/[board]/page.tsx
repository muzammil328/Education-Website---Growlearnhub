...
export async function generateMetadata({
  ...
  if (!book) {
    return {
      title: 'Class 10 Book Not Found | GrowLearnHub',
      description: 'Requested Class 10 book could not be found.',
    };
  }

  const bookName = removeDashAndUppercase(params.board);
  const canonical = `/class-10/books/${params.board}/`;
  const title = `${bookName} Class 10 Book PDF | Punjab Board`;
  const description = `${bookName} Class 10 textbook with English and Urdu medium options. Read online or choose your preferred medium for chapter-wise navigation.`;
  ...
  return generatePageMetadata({
    ...
    keywords: [
      ...
      `punjab board class 10 ${bookName.toLowerCase()} book`,
...
export default async function Page({ params }: { params: { board: string } }) {
  ...
  return (
    <UserLayout
      title={`${bookName} Class 10 Book PDF`}
      image="/10th/class_10_book.webp"
      canonical={canonical}
      url={url}
    >
      <article className="max-w-none">
        <section className="mb-8">
          <p className="lead text-foreground/90">
            Access <strong>{bookName}</strong> Class 10 textbook. Choose your preferred medium below
          ...
          <div className="my-6 grid grid-cols-1 gap-4 md:grid-cols-2">
            {book.mediums.map(medium => (
              <CardSmall
                ...
                link={`class-10/books/${book.slug}/${medium.slug}`}
              />
            ))}
          </div>
        </section>
            ...
            <div className="my-6 grid grid-cols-1 gap-4 md:grid-cols-2">
              {book.chapters.map(chapter => (
                <CardSmall
                  ...
                  title={chapter.name}
                  link={`class-10/books/${book.slug}/${chapter.mediumId || 'english-medium'}/${chapter.slug}`}
                />
              ))}
            </div>
          </section>
