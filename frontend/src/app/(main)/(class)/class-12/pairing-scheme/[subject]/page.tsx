...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  if (!classItem || !board || !subject) {
    return {
      title: 'Class 12 Pairing Scheme Not Found | GrowLearnHub',
      description: 'Requested Class 12 pairing scheme page could not be found.',
    };
  }

  const canonical = `/class-12/pairing-scheme/${board.slug}/${subject.slug}/`;
      ...
      <article className="max-w-none">
        ...
        <section className="mt-8">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {previous && (
              <Link
                href={`/class-12/pairing-scheme/${board.slug}/${previous.slug}`}
            ...
            <Link
              href={`/class-12/pairing-scheme/${board.slug}`}
              className="rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted"
            >
              All Subjects
            </Link>
            {next && (
              <Link
                href={`/class-12/pairing-scheme/${board.slug}/${next.slug}`}
                className="rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted"
              >
                Next: {next.name}
              </Link>
