...
export function generateMetadata({ params }: { params: Params }): Metadata {
  ...
  if (!classItem || !board) {
    return {
      title: 'Class 12 Pairing Scheme Board Not Found | GrowLearnHub',
      description: 'Requested Class 12 pairing scheme board page could not be found.',
    };
  }

  const canonical = `/class-12/pairing-scheme/${board.slug}/`;
      ...
      <article className="max-w-none">
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
            {board.subjects.map(subject => (
              <CardSmall
                key={subject.slug}
                title={subject.name}
                link={`class-12/pairing-scheme/${board.slug}/${subject.slug}`}
              />
            ))}
          </div>
        </section>
        ...
        <section className="mt-8">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <Link
              href="/class-12/pairing-scheme"
              className="rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted"
            >
              Back to Class 12 Pairing Scheme
            </Link>
          </div>
        </section>
      </article>
