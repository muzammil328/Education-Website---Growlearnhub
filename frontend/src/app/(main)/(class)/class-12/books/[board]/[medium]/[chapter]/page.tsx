...
export function generateStaticParams() {
  return CLASS12_PUNJAB_BOARDS.flatMap(board =>
    board.mediums.flatMap(medium =>
      getClass12Chapters(board.chapterCount).map(chapter => ({
        board: board.slug,
        medium: medium.slug,
...
export function generateMetadata({ params }: { params: Params }): Metadata {
  ...
  const chapter = board
    ? getClass12Chapters(board.chapterCount).find(item => item.slug === params.chapter)
    : undefined;

  if (!board || !medium || !chapter) {
    return {
      title: 'Class 12 Chapter Not Found | GrowLearnHub',
      description: 'Requested Class 12 chapter page could not be found.',
    };
  }

  const canonical = `/class-12/books/${board.slug}/${medium.slug}/${chapter.slug}/`;
  const url = `https://growlearnhub.com${canonical}`;
  const title = `${board.name} Class 12 ${chapter.title} (${medium.name}) | GrowLearnHub`;
  const description = `${chapter.title} of ${board.name} Class 12 (${medium.name}). Read chapter-wise guidance and view the full textbook PDF.`;

  return {
    title,
    description,
    keywords: [
      `${board.name} class 12 ${chapter.slug}`,
      `${board.name} ${medium.name.toLowerCase()} ${chapter.title.toLowerCase()}`,
      'class 12 chapter wise books',
    ],
...
export default function Page({ params }: { params: Params }) {
  const board = getClass12Board(params.board);
  const medium = board ? getClass12Medium(board, params.medium) : undefined;
  const chapters = board ? getClass12Chapters(board.chapterCount) : [];
      ...
      <article className="max-w-none">
        <section className="mb-8">
          ...
          <p className="text-foreground/80">
            You are viewing {chapter.title} from the {board.name} Class 12 textbook in {medium.name}
        ...
        <section className="mb-8">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {previousChapter && (
              <Link
                href={`/class-12/books/${board.slug}/${medium.slug}/${previousChapter.slug}`}
            ...
            <Link
              href={`/class-12/books/${board.slug}/${medium.slug}`}
              className="rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted"
            >
              All Chapters
            </Link>
            {nextChapter && (
              <Link
                href={`/class-12/books/${board.slug}/${medium.slug}/${nextChapter.slug}`}
                className="rounded-md border border-border px-4 py-2 text-sm font-medium text-foreground hover:bg-muted"
              >
