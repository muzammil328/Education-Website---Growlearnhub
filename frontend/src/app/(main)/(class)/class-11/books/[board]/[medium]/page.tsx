...
export function generateMetadata({ params }: { params: Params }): Metadata {
  ...
  if (!board || !medium) {
    return {
      title: 'Class 11 Book Medium Not Found | GrowLearnHub',
      description: 'Requested Class 11 book medium could not be found.',
    };
  }

  const canonical = `/class-11/books/${board.slug}/${medium.slug}/`;
  const url = `https://growlearnhub.com${canonical}`;
  const title = `${board.name} Class 11 Book PDF (${medium.name}) | GrowLearnHub`;
  const description = `Read ${board.name} Class 11 textbook in ${medium.name}. Use chapter-wise links or view the full PDF online.`;

  return {
    title,
    description,
    keywords: [
      `${board.name} class 11 ${medium.name.toLowerCase()} pdf`,
      `${board.name} class 11 book pdf`,
      'class 11 chapter wise book',
    ],
    ...
    openGraph: {
      ...
      images: [{ url: board.image, alt: `${board.name} class 11 ${medium.name}` }],
    },
    twitter: {
...
export default function Page({ params }: { params: Params }) {
  ...
  const chapters = getClass11Chapters(board.chapterCount);
  const canonical = `/class-11/books/${board.slug}/${medium.slug}/`;
  const url = `https://growlearnhub.com${canonical}`;

  return (
    <UserLayout
      title={`${board.name} Class 11 Book (${medium.name})`}
      image={board.image}
      canonical={canonical}
      url={url}
    >
      <article className="max-w-none">
        <section className="mb-8">
          <p className="lead text-foreground/90">
            Access {board.name} Class 11 textbook in {medium.name}. Use chapter-wise links below for
            quick navigation, or read the full PDF in one place.
          </p>
          <p className="text-foreground/80 mt-4">
            Go back to{' '}
            <Link
              href={`/class-11/books/${board.slug}`}
          ...
          <div className="my-6 grid grid-cols-1 gap-4 md:grid-cols-2">
            {chapters.map(chapter => (
              <CardSmall
                ...
                link={`class-11/books/${board.slug}/${medium.slug}/${chapter.slug}`}
              />
            ))}
          </div>
        </section>
