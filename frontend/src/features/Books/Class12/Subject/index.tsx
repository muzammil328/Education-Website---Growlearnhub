...
export default function SubjectClass12Book({ bookSlug }: { bookSlug: string }) {
  const { data, isLoading, error } = useBookDetail(CLASS_SLUG, bookSlug);
  const bookName = removeDashAndUppercase(bookSlug);

  const canonical = `/class-12/books/${bookSlug}/`;
  const url = `https://growlearnhub.com${canonical}`;

  if (isLoading) {
    return (
      <UserLayout title={`${bookName} Class 12 Book PDF`} image="/12th/class_12_book_growlearnhub.png" canonical={canonical} url={url}>
  ...
  if (error || !data?.data) {
    return (
      <UserLayout title="Book Not Found" image="/12th/class_12_book_growlearnhub.png" canonical={canonical} url={url}>
  ...
  return (
    <UserLayout
      title={`${displayName} Class 12 Book PDF | English & Urdu Medium`}
                  ...
                  {chapters.map((ch, idx) => (
                    <tr key={ch.slug} className="hover:bg-muted/40 transition-colors">
                      ...
                      <td className="px-4 py-3 font-medium text-foreground">
                        <Link href={`/class-12/books/${bookSlug}/${ch.slug}`} className="hover:text-primary hover:underline">
                          {ch.name}
                        </Link>
                      </td>
                      <td className="px-4 py-3">
                        <Link href={`/class-12/books/${bookSlug}/${ch.slug}`} className="inline-flex items-center rounded-md border border-primary px-3 py-1 text-xs font-medium text-primary hover:bg-primary hover:text-primary-foreground transition-colors">
                          {ch.name} — English
                        </Link>
                      </td>
                      <td className="px-4 py-3">
        ...
        <section className="border-t border-border pt-8 space-y-6 text-sm text-foreground/75 leading-relaxed">
          <div>
            <h2 className="text-xl font-semibold text-foreground mb-2">
              About the Class 12 {displayName} Textbook
            </h2>
            <p>
              The <strong>Class 12 {displayName}</strong> textbook follows the PCTB and FBISE
              syllabus for inter-part 2 students. It is the primary reference for students appearing
              in annual HSC-II board examinations across Punjab, Federal, and BISE regions.
            </p>
          </div>
          <div>
          ...
          <div>
            <h3 className="text-lg font-semibold text-foreground mb-2">
              How to Prepare {displayName} for Class 12 Board Exams
            </h3>
            <p>
              ...
              strategy for scoring well in 12th class {displayName.toLowerCase()} inter-part 2 board exams.
            </p>
          </div>
        </section>
