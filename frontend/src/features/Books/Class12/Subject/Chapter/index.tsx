...
export default function ChapterClass12Book({
  bookSlug,
  chapterSlug,
}: {
  bookSlug: string;
  ...
  const canonical = `/class-12/books/${bookSlug}/${chapterSlug}/`;
  const url       = `https://growlearnhub.com${canonical}`;

  if (isLoading) {
    return (
      <UserLayout title={`${chapterName} — ${bookName} Class 12`} image="/12th/class_12_book_growlearnhub.png" canonical={canonical} url={url}>
  ...
  return (
    <UserLayout
      title={`${chapterLabel}: ${displayChapter} — ${bookName} Class 12 PDF`}
    ...
    >
      <article className="max-w-none space-y-10">
        ...
        <nav className="flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
          <Link href="/class-12/books" className="hover:text-primary hover:underline">Class 12 Books</Link>
          <span>/</span>
          <Link href={`/class-12/books/${bookSlug}`} className="hover:text-primary hover:underline">{bookName}</Link>
          <span>/</span>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-3">
              {otherChapters.map((ch, idx) => (
                <Link
                  key={ch.slug}
                  href={`/class-12/books/${bookSlug}/${ch.slug}`}
