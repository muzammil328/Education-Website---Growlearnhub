...
export default function ChapterClass9Book({
  bookSlug,
  chapterSlug,
}: {
  bookSlug: string;
  ...
  const canonical = `/class-9/books/${bookSlug}/${chapterSlug}/`;
  const url       = `https://growlearnhub.com${canonical}`;

  if (isLoading) {
    return (
      <UserLayout title={`${chapterName} — ${bookName} Class 9`} image="/9th/class_9_book_growlearnhub.png" canonical={canonical} url={url}>
  ...
  if (error || !data?.data) {
    return (
      <UserLayout title="Chapter Not Found" image="/9th/class_9_book_growlearnhub.png" canonical={canonical} url={url}>
  ...
  return (
    <UserLayout
      title={`${chapterLabel}: ${displayChapter} — ${bookName} Class 9 PDF`}
    ...
    >
      <article className="max-w-none space-y-10">
        ...
        <nav className="flex flex-wrap items-center gap-1.5 text-xs text-muted-foreground">
          <Link href="/class-9/books" className="hover:text-primary hover:underline">Class 9 Books</Link>
          <span>/</span>
          <Link href={`/class-9/books/${bookSlug}`} className="hover:text-primary hover:underline">{bookName}</Link>
          <span>/</span>
        ...
        <section className="border-t border-border pt-8 space-y-5 text-sm text-foreground/75 leading-relaxed">
          <div>
            ...
            <p>
              <strong>{chapterLabel}: {displayChapter}</strong> is part of the Class 9{' '}
              <strong>{bookName}</strong> curriculum prescribed by Punjab Board (PCTB) and Federal
              Board (FBISE). This chapter covers key concepts that appear regularly in annual board
              exams. Studying it chapter by chapter — along with exercises and past paper questions
              — is the most effective strategy for scoring well in 9th class {bookName.toLowerCase()}.
            </p>
          </div>
          <div>
            ...
            <p>
              ...
              this chapter to consolidate your understanding before the exam.
            </p>
          </div>
          <div>
            <h3 className="text-lg font-semibold text-foreground mb-2">
